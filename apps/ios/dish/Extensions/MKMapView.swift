...
extension MKMapView {
  ...
  func latitudeToPixelSpaceY(latitude: Double) -> Double {
    return round(
      Double(
        Float(MERCATOR_OFFSET()) - Float(MERCATOR_RADIUS())
          * logf(
            (1 + sinf(Float(latitude * .pi / 180.0))) / (1 - sinf(Float(latitude * .pi / 180.0))))
          / Float(2.0)))
  }
  ...
  func pixelSpaceYToLatitude(pixelY: Double) -> Double {
    return (.pi / 2.0 - 2.0 * atan(exp((round(pixelY) - MERCATOR_OFFSET()) / MERCATOR_RADIUS())))
      * 180.0 / .pi
  }

  func coordinateSpanWithMapView(centerCoordinate: CLLocationCoordinate2D, zoomLevel: Double)
    -> MKCoordinateSpan
  {
    ...
    print(
      "final span deltas \(mapSizeInPixels.width) \(zoomScale) \((scaledMapWidth,scaledMapHeight)) \((minLng, maxLng)) \(latitudeDelta) \(longitudeDelta)"
    )
  ...
  func setCenterCoordinate(
    centerCoordinate: CLLocationCoordinate2D, zoomLevel: Double, animated: Bool
  ) {
    // clamp large numbers to 28
    let zoomLevel = min(zoomLevel, 28)
    // use the zoom level to compute the region
    let span = self.coordinateSpanWithMapView(
      centerCoordinate: centerCoordinate, zoomLevel: zoomLevel)
