...
struct DishScrollView<Content>: UIViewRepresentable where Content: View {
  ...
  func updateUIView(
    _ uiView: UIScrollView, context: UIViewRepresentableContext<DishScrollView<Content>>
  ) {
...
final class DishScrollViewVC<Content>: UIViewController where Content: View {
  ...
  func add(content: Content) {
    ...
    self.contentView.leadingAnchor.constraint(equalTo: self.scrollView.leadingAnchor).isActive =
      true
    self.contentView.trailingAnchor.constraint(equalTo: self.scrollView.trailingAnchor).isActive =
      true
...
extension DishScrollViewVC: UIViewControllerRepresentable {
  func makeUIViewController(context: UIViewControllerRepresentableContext<DishScrollViewVC>)
    -> DishScrollViewVC
  {
  ...
  func updateUIViewController(
    _ uiViewController: DishScrollViewVC,
    context: UIViewControllerRepresentableContext<DishScrollViewVC>
  ) {
    // Do nothing at the moment.
  }
}
