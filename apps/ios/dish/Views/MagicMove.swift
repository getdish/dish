...
struct MagicMove<Content>: View where Content: View {
  ...
  var body: some View {
    ...
    let keys = magicItemsStore.startItems
      .filter { $0.value != nil }
      .filter { startItem in
        magicItemsStore.endItems.map { $0.value }.contains(where: { $0?.id == startItem.value!.id })
      }
      .map { $0.key }
      .sorted()
...
struct MagicItem<Content>: View where Content: View {
  ...
  init(
    _ id: String, at: MagicItemPosition, disableTracking: Bool = false,
    @ViewBuilder content: @escaping () -> Content
  ) {
    self.id = id
    self.at = at
  ...
  var body: some View {
    return self.content
      ...
      .opacity(
        self.store.state == .animate ? OFF_OPACITY : self.store.position == at ? 1 : OFF_OPACITY
      )
      .onDisappear {
        if self.at == .start {
