...
const statsFile = Path.resolve(Path.join(rootDir, 'web-build-ssr/loadable-stats.json'));
...
server.get('*', async (req, res) => {
    ...
    const clientScripts = clientHTML.match(/<script\b[^>]*>([\s\S]*?)<\/script>/gm) ?? [];
...
function cors() {
    const HEADER_ALLOWED = 'Origin, X-Requested-With, Content-Type, Accept, Authorization, X-Token, Access-Control-Allow-Headers';
    return (req, res, next) => {
