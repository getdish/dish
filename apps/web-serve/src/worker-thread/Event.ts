...
const touchListFromTransfer = (
  ...
  if (event[key] !== undefined) {
    ...
    touchListKeys.forEach((touchListKey) => {
      ...
      const transferredTouch = (event[key] as TransferrableTouchList)[
        numericKey
      ]
      list[numericKey] = {
        ...
        target: get(
          transferredTouch[7] !== 0
            ? transferredTouch[7]
            : document[TransferrableKeys.index]
        ),
      }
    })
