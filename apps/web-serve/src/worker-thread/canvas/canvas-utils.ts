...
export function retrieveImageBitmap(
  ...
  return new Promise((resolve) => {
    const messageHandler = ({ data }: { data: ImageBitmapToWorker }) => {
      if (
        ...
        data[TransferrableKeys.callIndex] === callIndex
      ) {
        document.removeGlobalEventListener('message', messageHandler)
        const transferredImageBitmap = (data as ImageBitmapToWorker)[
