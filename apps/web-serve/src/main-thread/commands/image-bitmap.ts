...
import {
  ImageBitmapMutationIndex,
  TransferrableMutationType,
} from '../../transfer/TransferrableMutation'
import { CommandExecutorInterface } from './interface'

export const ImageBitmapProcessor: CommandExecutorInterface = (
  strings,
  nodeContext,
  workerContext,
  objectContext,
  ...
  return {
    execute(
      ...
      if (allowedExecution && allowedMutation) {
        const targetIndex =
          mutations[startPosition + ImageBitmapMutationIndex.Target]
        const target = nodeContext.getNode(targetIndex)
        if (target) {
          self
            .createImageBitmap(target as HTMLImageElement | HTMLCanvasElement)
            .then((imageBitmap) => {
              workerContext.messageToWorker(
                {
                  [TransferrableKeys.type]: MessageType.IMAGE_BITMAP_INSTANCE,
                  [TransferrableKeys.callIndex]:
                    mutations[
                      startPosition + ImageBitmapMutationIndex.CallIndex
                    ],
    ...
    print(mutations: Uint16Array, startPosition: number): Object {
      ...
      return {
        ...
        allowedExecution,
        callIndex:
          mutations[startPosition + ImageBitmapMutationIndex.CallIndex],
      }
    },
  }
}
