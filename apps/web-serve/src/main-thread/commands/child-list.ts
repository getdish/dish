...
export const ChildListProcessor: CommandExecutorInterface = (
  strings,
  { getNode }: NodeContext,
  workerContext,
  objectContext,
  ...
  return {
    execute(
      ...
      const appendNodeCount =
        mutations[startPosition + ChildListMutationIndex.AppendedNodeCount]
      const removeNodeCount =
        mutations[startPosition + ChildListMutationIndex.RemovedNodeCount]
      if (allowedExecution && allowedMutation) {
        const targetIndex =
          mutations[startPosition + ChildListMutationIndex.Target]
        ...
        if (target) {
          ...
          if (appendNodeCount > 0) {
            mutations
              .slice(
                startPosition + ChildListMutationIndex.Nodes,
                startPosition + ChildListMutationIndex.Nodes + appendNodeCount
              )
              .forEach((addId) => {
                const nextSibling =
