...
test('will append child when ref is null or undefined', (t) => {
  const { node, child, childTwo } = t.context
  t.is(
    node.insertBefore(child, undefined),
    child,
    'inserting child before undefined ref returns the appended child'
  )
  t.deepEqual(
    node.childNodes[0],
    child,
    'child is appended when ref is undefined in insertBefore'
  )
  t.is(
    node.insertBefore(childTwo, null),
    childTwo,
    'inserting child before null ref returns the appended child'
  )
  t.deepEqual(
    node.childNodes[1],
    childTwo,
    'child is appended when ref is null in insertBefore'
  )
})
...
test('will NOOP when requested to insert a child before the same child', (t) => {
  ...
  t.deepEqual(
    ...
    child,
    'returns the child that passed as both arguments'
  )
  t.is(
    node.childNodes.indexOf(child),
    0,
    'position of child inserted remains when Node.insertBefore'
  )
})
...
test('will insert a child before ref', (t) => {
  ...
  t.deepEqual(
    ...
    childTwo,
    'will return childTwo inserted before child'
  )
  t.is(
    node.childNodes.indexOf(childTwo),
    0,
    'childTwo was inserted before ref (child)'
  )
})
...
test('will insert a child in the middle of Node.childNodes, when ref is later within Node.childNodes', (t) => {
  ...
  t.is(
    node.childNodes.indexOf(childThree),
    node.childNodes.indexOf(childTwo) - 1,
    'childThree was inserted before childTwo'
  )
})
