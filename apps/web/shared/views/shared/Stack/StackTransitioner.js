...
export default class StackTransitioner extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    const [children, previousChildren] = this.getChildren(
      nextProps.location,
      location
    )
    ...
    if (
      ...
      (nextProps.animationType === NONE && !routeAnimationType) ||
        ...
        () => {
          ...
          this.animation = Animated.timing(this.animatedValue, {
            duration: getDuration(
              routeAnimationType || nextProps.animationType,
              action
            ),
  ...
  isFixed(element) {
    ...
    return (
      children.props[`${element}Component`] ===
      previousChildren.props[`${element}Component`]
    ) // both routes have same fixed headerComponent or footerComponent
  }
  ...
  render() {
    ...
    const PreviousHeader =
      previousChildren && previousChildren.props.headerComponent
    const PreviousFooter =
      previousChildren && previousChildren.props.footerComponent
        ...
        routes.push(
          <Animated.View key={children.key} style={this.getRouteStyle(1)}>
            {!this.isFixed('footer') && Footer && (
              <Footer animatedValue={this.animatedValue} />
            )}
            {children}
