...
import ITsccSpecJSON, {
  IDebugOptions,
  IModule,
  INamedModuleSpecs,
} from './ITsccSpecJSON'
import { CycleError, DirectedTree } from './shared/Graph'
...
export type IInputTsccSpecJSON =
  | IInputTsccSpecJSONWithOptionalSpecFile
  | IInputTsccSpecJSONWithSpecFile

function hasSpecFileKey(
  json: IInputTsccSpecJSON
): json is IInputTsccSpecJSONWithSpecFile {
...
export default class TsccSpec implements ITsccSpec {
  ...
  private static PATH_SEP =
    ...
    ']'
  private static readonly RE_DOT_PATH = new RegExp(
    '^[\\.]{1,2}' + TsccSpec.PATH_SEP
  )
  private static readonly RE_ENDS_WITH_SEP = new RegExp(TsccSpec.PATH_SEP + '$')
  private static isDotPath(p: string) {
    return TsccSpec.RE_DOT_PATH.test(p)
  }
  private static endsWithSep(p: string) {
    return TsccSpec.RE_ENDS_WITH_SEP.test(p)
  }
  /**
  ...
  private static findTsccSpecAndThrow(root: string): string {
    const specPath = TsccSpec.resolveSpecFile(
      root,
      TsccSpec.SPEC_FILE,
      process.cwd()
    )
    ...
    const readSpecJSON = (): ITsccSpecJSON => {
      try {
        ...
        throw new TsccSpecError(
          `Spec file is an invalid JSON: ${TsccSpec.toDisplayedPath(
            tsccSpecJSONPath
          )}.`
        )
      }
  ...
  private computeOrderedModuleSpecs() {
    ...
    if (Array.isArray(modules)) {
      ...
      this.orderedModuleSpecs = modules.map((module) =>
        this.interopModuleSpecs(module.moduleName, module)
      )
      return
    }
    ...
    try {
      ...
      if (e instanceof CycleError) {
        throw new TsccSpecError(
          `Circular dependency in modules ${[...e.cycle]}`
        )
      }
    }
