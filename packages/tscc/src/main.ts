...
import tscc, { CcError, TEMP_DIR } from './tscc'
...
export function parseTsccCommandLineArgs(
  args: string[],
  strict = true
...
export function buildTsccSpecJSONAndTsArgsFromArgs(args) {
  ...
  const closureCompilerArgs: string[] =
    (<any>yargs().parserConfiguration({ 'populate--': true }).parse(tsArgs))[
      '--'
    ] || []
  ...
  main(parsedArgs)
    ...
    .catch((e) => {
      if (e instanceof TsccSpecError) {
        ...
        tsccWarning.log(
          chalk.red(`The compilation has terminated with an unexpected error.`)
        )
