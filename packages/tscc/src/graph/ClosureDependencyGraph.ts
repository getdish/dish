import { INamedModuleSpecsWithId } from '@dish/tscc-spec'
...
import {
  ClosureSourceError,
  sourceNodeFactoryFromContent,
} from './source_node_factory'
...
export default class ClosureDependencyGraph {
  async addSourceByFileNames(
    fileNames: string[],
    fsCacheAccessor: FSCacheAccessor<ISourceNode>
  ) {
    await Promise.all(
  ...
  getSortedFilesAndFlags(
    entryPoints: Omit<INamedModuleSpecsWithId, 'entry'>[]
  ): IFilesAndFlags {
    let sortedFileNames = entryPoints.map((entryPoint) => {
      ...
      if (entryPoint.moduleId === null) {
        ...
        deps = [...this.getRequiredNodes(entryPoint.moduleId)].map(
          ClosureDependencyGraph.getFileName
        )
      }
    ...
    let forwardDeclaredFileNames = [...this.forwardDeclared].map(
      ClosureDependencyGraph.getFileName
    )
    ...
    const flags =
      entryPoints.length === 1
        ...
        : riffle(
            '--chunk',
            sortedFileNames.map((depsOfAModule, index) => {
              let entryPoint = entryPoints[index]
              const args: (string | number)[] = [
                entryPoint.moduleName,
                depsOfAModule.length,
              ]
              if (index !== 0) {
