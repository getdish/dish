 ...
 */
export async function sourceNodeFactory(
  closureSourcePath: string
): Promise<ISourceNode> {
...
export function sourceNodeFactoryFromContent(
  fileName: string,
  content: string
): ISourceNode {
...
class ClosureSourceLineParser {
  ...
  constructor(private closureSourcePath: string) {}
    ...
    if (reGoogModule.exec(line)) {
      if (this.moduleSymbol) {
        throw new ClosureSourceError(
          `Duplicate module symbols in ${this.closureSourcePath}`
        )
      }
  ...
  getSourceNode(): ISourceNode {
    ...
    return {
      ...
      provides: this.moduleSymbol
        ? [this.moduleSymbol]
        : [...this.providedSymbols],
      // goog is implicitly required by every module
      required: this.providedSymbols.has('goog')
        ? []
        : ['goog', ...this.requiredSymbols],
