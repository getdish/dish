...
export class ClosureJsonToVinyl extends LoggingTransformStream {
  constructor(private applySourceMap: boolean, logger: Logger) {
    super(logger)
  }
  ...
  async _rawTransform(data: Vinyl, encoding) {
    ...
    if (!data[SOURCE_MAP]) {
      ...
      data.contents = Buffer.from(
        origContents.replace(RemoveTempGlobalAssignments.reCcExport, '')
      )
    } else {
      // Perform sourcemap-aware replace
      const origMap: RawSourceMap = data[SOURCE_MAP]
      const { contents: replacedContents, intervals } = splitWithRegex(
        origContents,
        RemoveTempGlobalAssignments.reCcExport
      )
      const replacedMap = await spliceSourceMap(
        origContents,
        origMap,
        intervals
      )
