...
export default function decoratorPropertyTransformer(
  tsickleHost: TsickleHost
): (context: ts.TransformationContext) => ts.Transformer<ts.SourceFile> {
  return (context: ts.TransformationContext) => {
    return (sf: ts.SourceFile) => {
      return new DecoratorTransformer(
        tsickleHost,
        context,
        sf
      ).transformSourceFile()
...
class DecoratorTransformer extends TsHelperTransformer {
  ...
  protected onHelperCall(
    node: ts.CallExpression,
    googReflectImport: ts.Identifier
  ) {
    ...
    const googReflectObjectProperty = ts.setTextRange(
      ts.createCall(
        ...
        undefined,
        [ts.createStringLiteral(propName), ts.getMutableClone(target)]
      ),
      propNameLiteral
    )
    ...
    this.tempGlobalAssignments.push(
      ts.setEmitFlags(
        ...
        ts.EmitFlags.NoSourceMap |
          ts.EmitFlags.NoTokenSourceMaps |
          ts.EmitFlags.NoNestedSourceMaps
      )
    )
    return newCallExpression
  }

  protected combineStatements(
    stmts: ts.Statement[],
    googReflectImport: ts.Identifier
  ) {
    ...
    stmts.push(
      ts.createExpressionStatement(
        ts.createStringLiteral('__tscc_export_start__')
      ),
      ts.createBlock(this.tempGlobalAssignments),
      ts.createExpressionStatement(
        ts.createStringLiteral('__tscc_export_end__')
      )
    )
    return stmts
  }
