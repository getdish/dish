...
export class TsError extends Error {
  constructor(public diagnostics: ReadonlyArray<ts.Diagnostic>) {
...
export default class TsccSpecWithTS extends TsccSpec
  implements ITsccSpecWithTS {
  static loadTsConfigFromArgs(
    tsArgs: string[],
    specRoot: string,
    onWarning: TWarningCallback
  ) {
    ...
    const configFileName = TsccSpecWithTS.findConfigFileAndThrow(
      options.project,
      specRoot
    )
  ...
  private static findConfigFileAndThrow(
    ...
    const configFileName = TsccSpecWithTS.resolveSpecFile(
      searchPath,
      'tsconfig.json',
      defaultLocation
    )
    if (configFileName === undefined) {
      throw new TsccSpecError(
        `Cannot find tsconfig at ${TsccSpecWithTS.toDisplayedPath(searchPath)}.`
      )
    }
  ...
  static loadSpecWithTS(
    ...
    let { tsccSpecJSON, tsccSpecJSONPath } = TsccSpecWithTS.loadSpecRaw(
    ...
    let { projectRoot, parsedConfig } = Array.isArray(tsConfigPathOrTsArgs)
      ? TsccSpecWithTS.loadTsConfigFromArgs(
          tsConfigPathOrTsArgs,
          specRoot,
          onTsccWarning
        )
      : TsccSpecWithTS.loadTsConfigFromPath(
          tsConfigPathOrTsArgs,
          specRoot,
  ...
  private static pruneCompilerOptions(
    ...
    if (options.outDir) {
      onWarning(
        `--outDir option is ignored. Use prefix option in the spec file.`
      )
    ...
    if (!options.importHelpers) {
      onWarning(
        `tsickle uses a custom tslib optimized for closure compiler. importHelpers flag is set.`
      )
      options.importHelpers = true
    }
    if (options.removeComments) {
      onWarning(
        `Closure compiler relies on type annotations, removeComments flag is unset.`
      )
      options.removeComments = false
    }
    if (options.inlineSourceMap) {
      onWarning(
        `Inlining sourcemap is not supported. inlineSourceMap flag is unset.`
      )
  ...
  private tsCompilerHost: ts.CompilerHost = ts.createCompilerHost(
  ...
  getOutputFileNames(): string[] {
    return this.getOrderedModuleSpecs().map((moduleSpec) => {
  ...
  private getDefaultFlags(): { [flag: string]: primitives | primitives[] } {
    ...
    const defaultFlags = {}
    defaultFlags['language_in'] =
      TsccSpecWithTS.tsTargetToCcTarget[
    ...
    if (this.getOrderedModuleSpecs().length > 1) {
      ...
      defaultFlags['chunk_output_path_prefix'] = this.relativeFromCwd(
        this.getOutputPrefix('cc')
      )
    } else {
      // Single-chunk build uses --js_output_file.
      defaultFlags['js_output_file'] =
        this.relativeFromCwd(this.getOutputPrefix('cc')) +
        this.getOrderedModuleSpecs()[0].moduleName +
        '.js'
    }
  ...
  getAbsoluteFileNamesSet() {
    return new Set(
      this.parsedConfig.fileNames.map((fileName) =>
  ...
  getProjectHash(): string {
    return require('crypto')
      ...
      .update(
        this.basePath +
          JSON.stringify(this.tsccSpec) +
          this.projectRoot +
          JSON.stringify(this.parsedConfig.options)
      )
      .digest('hex')
  }
}
