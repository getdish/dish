  ...
  ({
    ...
    return (
      ...
      >
        ...
        <AbsoluteVStack fullscreen opacity={0.8} transform={[{ scale: 0.95 }]}>
          <CircularProgress
            fill={ratio * 100}
            size={sizePx}
            width={2}
            tintColor={ratio < 0.6 ? red : ratio < 0.8 ? orange : green}
            lineCap="round"
            arcSweepAngle={300}
            rotation={180 + (1 - ratio) * 270}
          />
        </AbsoluteVStack>
        ...
        <Text
          ...
          letterSpacing={-0.5}
          color={score > 0 && ratio > 0.5 ? darkGreen : darkRed}
        >
          {score ?? ''}
        </Text>
        {subtle ? (
